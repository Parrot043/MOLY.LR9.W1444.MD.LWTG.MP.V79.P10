...
kal_bool usim_select_power(usim_power_enum ExpectVolt, sim_HW_cb *hw_cb);
void usim_activation(sim_HW_cb *hw_cb);
...
global_interface:
	...
	/* fix FTA 5.1.5.4 fail, should retry 3V without return here */
//	else{ //from latest 7816-3, if ATR is valid without class indicator, host should continue normal operation
//		return USIM_NO_ERROR;
//	}	
#endif	
...
kal_bool usim_select_power(usim_power_enum ExpectVolt, sim_HW_cb *hw_cb)
{
   ...
   SIM_SetAtime(0xf000):
	...
	while(retry++ < 3)
	{
		...
		USIM_WAIT_EVENT_MTK(usim_dcb); //MUST necessary
#if defined SIM_DRV_IC_USB
...
/*old method, just copy here and will change it when necessary*/
...
#if defined (SIM_PMIC_GPIO_POLLING_MODE)
	kal_sprintf(sim_dbg_str,"DRV_Read_PMIC_GPIO_DataT9(0x0010) %x", DRV_ICC_PMU_GetGPIO(hw_cb->simInterface, hw_cb->simPmicHwCb->gpio_dir_base));
   ...
   dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
	...
	kal_sprintf(sim_dbg_str,"DRV_Read_PMIC_GPIO_Data(0x00D8) %x", DRV_ICC_PMU_GetGPIO(hw_cb->simInterface, hw_cb->simPmicHwCb->gpio_mode_base));
#ifdef ATEST_DRV_ENABLE	
   dbg_print(sim_dbg_str);
#else	
   dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
	...
	kal_sprintf(sim_dbg_str,"DRV_Read_PMIC_GPIO_Data(0x0100) %x", DRV_ICC_PMU_GetGPIO(hw_cb->simInterface, hw_cb->simPmicHwCb->gpio_ap_mode_base));
#ifdef ATEST_DRV_ENABLE	
   dbg_print(sim_dbg_str);
#else	
   dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
...
//		      PMIC_DRV_SetGpioData(0x0092,0xFFFF,0x4000,0x0);
			...
			kal_sprintf(sim_dbg_str,"DRV_Read_PMIC_GPIO_Data-0(0x0090) %x %x", DRV_ICC_PMU_GetGPIO(hw_cb->simInterface, hw_cb->simPmicHwCb->gpio_dout_base), drv_get_current_time());
#ifdef ATEST_DRV_ENABLE	
			dbg_print(sim_dbg_str);
#else	
			dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
...
//		PMIC_DRV_SetGpioData(0x0094,0x4000,0x4000,0x0);
		...
		kal_sprintf(sim_dbg_str,"DRV_Read_PMIC_GPIO_Data-usim_deact(0x0090) %x", DRV_ICC_PMU_GetGPIO(hw_cb->simInterface, hw_cb->simPmicHwCb->gpio_dout_base));
#ifdef ATEST_DRV_ENABLE	
		dbg_print(sim_dbg_str);
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
	...
	/*turn off LDO*/
	   kal_sleep_task(50);	
			...
			kal_sleep_task(10);
#if defined (__SIM_DRV_SET_STB_SIO__)
			//HW enable used to fix SIO pluse signal
	...
	kal_sleep_task(16);
	   ...
	   {
		   reg = SIM_Reg(SIM0_BASE_ADDR_MTK + 0x40);			
		   SIM_WriteReg(SIM0_BASE_ADDR_MTK + 0x40, 0x0);			
		   SIM_WriteReg(SIM0_BASE_ADDR_MTK + 0x40, 0x5aa5);				
		   DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC20, hw_cb->mtk_baseAddr,SIM_Reg(SIM0_BASE_ADDR_MTK + 0x40),SIM_Reg32(0x80000450),hw_cb->mtk_pdnAddr,0x1116);			
		   SIM_WriteReg(SIM0_BASE_ADDR_MTK + 0x40, reg);
			/* read whole register */	
...
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "T1 switched ");
#endif
...
#if defined (__SIM_DVT__)
#ifdef ATEST_DRV_ENABLE	
            dbg_print("CMD_RX_STATE ");
#else	
            dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "CMD_RX_STATE ");
#endif
...
/* under construction !*/
				...
				*/
				...
				if(usim_dcb->ev_status == USIM_RX_INVALID)
				{
					...
					kal_sprintf(sim_dbg_str,"usim_dcb->ev_status Status:%x",usim_dcb->ev_status);
#ifdef ATEST_DRV_ENABLE	
					dbg_print(sim_dbg_str);
#else	
					dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
   ...
   dbg_print("\r\n");
#else	
   dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "\r\n");
#endif
...
#if defined (__SIM_DVT__)
	kal_sprintf(sim_dbg_str,"int Status:%x",int_status);
#ifdef ATEST_DRV_ENABLE	
   dbg_print(sim_dbg_str);
#else	
   dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
   ...
   kal_uint16 i;
	...
	USIM_SET_EVENT_Multiple(usim_dcb);
				...
				else
				{
					...
					kal_sprintf(sim_dbg_str,"[SIM_DRV]Card Error, Enter find Card Process.\n\r");
#ifdef ATEST_DRV_ENABLE	
					dbg_print(sim_dbg_str);
#else	
					dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
				   ...
				   kal_sprintf(sim_dbg_str,"[SIM_DRV]Check HW Waveform for HW issue. TS:%x T0:%x usim_dcb->ev_status:%x, int_status:%x",TS, T0, usim_dcb->ev_status, int_status);
#ifdef ATEST_DRV_ENABLE	
               dbg_print(sim_dbg_str);
#else	
               dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
...
PREFER_3V:
	...
	*ResultVolt = usim_dcb->power;
	...
	//if(usim_dcb->reset_mode == USIM_RESET_NEGOTIABLE)
	{
	   ...
	   /* disable DMA clk gating */
		DMA_SleepDisable(usim_dcb->dma_port);	
#endif	
...
#if defined (__DMA_LOW_POWER__)
	/* enable DMA clk gating */
	DMA_SleepEnable(usim_dcb->dma_port);
#endif		
	}
...
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "sim_baud8N4Test");
#endif
...
sim_reg_readCheck(kal_uint32 *regAddr, kal_uint32 regValue, char *testItem)
{
	...
	if(regValue != simReg){
		...
		kal_sprintf(sim_dbg_str, "Error:address %x read back %x while expecting %x\n\r", (kal_uint32)regAddr, simReg, regValue);
#ifdef ATEST_DRV_ENABLE	
		dbg_print(sim_dbg_str);
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, sim_dbg_str);
...
void sim_reg_RAWCheck(sim_HW_cb *hw_cb)
{
	...
	sim_reg_readCheck((kal_uint32 *)(SIM0_BASE_ADDR_MTK+ SIM_INS_MTK),			0x00000055,	"RAWCheck_28");
	DRV_WriteReg((SIM0_BASE_ADDR_MTK+ SIM_INS_MTK), 0x00000000);
...
//	return USIM_NO_INSERT;
	...
	if(warm == KAL_FALSE)
	{
		...
		else
		{
			...
			if(SIM_Reg(SIM0_BASE_ADDR_MTK + 0x0070) & SIM_ATR_STA_AL){
			...
			else if(SIM_Reg(SIM0_BASE_ADDR_MTK + 0x0070) & SIM_ATR_STA_IR){
			...
			else
			{
				...
				DRV_ICC_print(SIM_PRINT_L1SIM_CMD_TRC43, SIM_Reg(SIM0_BASE_ADDR_MTK + 0x24),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x34),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x60),SIM_Reg(SIM0_BASE_ADDR_MTK + 0x68),0);		
...
#if defined (__DMA_LOW_POWER__)
	/* disable DMA clk gating */
	DMA_SleepDisable(usimCard->dma_port);	
#endif	
...
#if defined (__DMA_LOW_POWER__)
		/* enable DMA clk gating */
		DMA_SleepEnable(usimCard->dma_port);
#endif	
		return result;
	}
	else
