...
//	SimCard->gpt_handle = usim_dcb->gpt_handle;
	...
	if (SimCard->result == SIM_SUCCESS)
	{
		...
		while(1)
		{
			...
			{
#ifdef ATEST_DRV_ENABLE	
				dbg_print("Bad card/Hw contact issue, cause PTS error. Enter recovery process");
#else	
				dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "Bad card/Hw contact issue, cause PTS error. Enter recovery process");
#endif
				...
				{
#ifdef ATEST_DRV_ENABLE	
						dbg_print("RETRY_18V_ONLY_USIM");
#else	
						dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "RETRY_18V_ONLY_USIM");
#endif
...
#else	
					dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "RETRY_3V_ON_PTS_ERROR");
#endif
				...
				if(KAL_TRUE != returnBool){
					//dbg_print("[SIM]:resetNoATR failed, hahahaahahahahahhahaahhahahahahhahaahhhhhhhhhhhhhhhhhhxxxxxxxxxxx");
	...
	if(SimCard->simMagic1 != SimCard->simMagic2){ /*cmd finished before GPT timeout*/
		...
		if(0x60 == SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_SW1_MTK)){
			...
			if(88 < SimCard->sim_nullByteIssuenonNullCount){
				...
				SIM_DisAllIntr();
#ifdef ATEST_DRV_ENABLE	
					 dbg_print("[SIM] : non null timeout\n\r");
#else	
					 dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "[SIM] : non null timeout\n\r");
#endif
...
/* under construction !*/
   ...
   *Error = KAL_FALSE;
   ...
   if (expSize == 0)
   {
		  ...
		  dbg_print("SIM TX");
#else	
		  dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "SIM TX");
#endif
	...
	if(SimCard->sim_input.count > SIM_TX_DELAY_LEN && SimCard->Speed > Speed372)
     ...
     SimCard->sim_input.callback = NULL;   /*4*/
		...
		if(10 == sim_TXTimes){
			sim_TXTimes = 0;
#ifdef ATEST_DRV_ENABLE	
		dbg_print("make TX disable");
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "make TX disable");
#endif
   ...
   else
   {
		...
		dbg_print("SIM RX");
#else	
		dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "SIM RX");
#endif
...
//	 	DMA_WaitUntilRdy(SimCard->sim_dmaport);   //a while loop to check running status.
   ...
   *rcvSize = SimCard->recDataLen;
   ...
   #if defined(SIM_DEBUG_INFO)
#ifdef ATEST_DRV_ENABLE	
		 dbg_print("SIM_SUCCESS");
#else	
		 dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, "SIM_SUCCESS");
#endif
