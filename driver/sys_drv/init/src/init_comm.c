...
/*************************************************************************
* FUNCTION
*  INT_DenaliPlusVersion
*
* DESCRIPTION
*  Due to denali_1/1+/2/2+/3 share modem load.
*  we use INT_DenaliVersion() and MSB of AP Segment Code
*  to distinguish which chip is currently.
*  
* PARAMETERS
*  none
*
...
*************************************************************************/
#if defined(MT6735)
NEWCHIP_NAME INT_DenaliPlusVersion(void)
{
    #define SEGMENT_CODE ((kal_uint32*)0xA02061B0)
    volatile kal_uint32 MSB_EFuse;
    // Read segment code to parse efuse bit
    MSB_EFuse = (*SEGMENT_CODE) >> 31; 

    if ((INT_DenaliVersion() == E_DENALI_1) && (MSB_EFuse == 0))
    {   //For Denali-1 HW Code is 0x6735, HW SubCode is 0x8A00, MSB of SEGMENT_CODE is 0
        return DENALI_1;
    }
    else if ((INT_DenaliVersion() == E_DENALI_1) && (MSB_EFuse == 1))
    {   //For Denali-1+ HW Code is 0x6735, HW SubCode is 0x8A00, MSB of SEGMENT_CODE is 1
        return DENALI_1_PLUS;
    }
    else if ((INT_DenaliVersion() == E_DENALI_2) && (MSB_EFuse == 0))
    {   //For Denali-2 HW Code is 0x6735, HW SubCode is 0x8B00, MSB of SEGMENT_CODE is 0
        return DENALI_2;
    }
    else if ((INT_DenaliVersion() == E_DENALI_2) && (MSB_EFuse == 1))
    {   //For Denali-2+ HW Code is 0x6735, HW SubCode is 0x8B00, MSB of SEGMENT_CODE is 1
        return DENALI_2_PLUS;
    }
    else if (INT_DenaliVersion() == E_DENALI_3)
    {   //For Denali-3 HW Code is 0x6753, HW SubCode is 0x8A00
        return DENALI_3;
    }
    else
