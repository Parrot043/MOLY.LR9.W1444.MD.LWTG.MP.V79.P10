...
*
*************************************************************************/
#if defined(__MTK_TARGET__) && (!(defined(MT6290) || defined(MT6595))) /* MT6290/MT6595 use different TCM address */
...
{
    PW_CTRL_GET_POWERON_REASON CtrlVal = {UNKNOWN_PWRON};
    DCL_HANDLE handle;
...
/* mtk02439: Version register cannot be seen at MD side on MT6575 and MT6577 */
   ...
   if (majrev == 0x0a) {   /* directiver A, which often skip the directive */
      snprintf(str_return, sizeof(str_return), "MT%4d, E%02d", temp_num, ffix);
   }
   else {
      snprintf(str_return, sizeof(str_return), "MT%4d%1X, E%02d", temp_num, majrev, ffix);
   }
#endif
...
kal_char* release_bb_sw_ver(void)
{
    ...
    snprintf(str_return, sizeof(str_return), "S%02d", chipSWVer);
