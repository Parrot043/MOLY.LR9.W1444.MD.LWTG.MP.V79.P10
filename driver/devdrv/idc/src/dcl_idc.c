...
*	8. IDC_CMD_SEND_EVENT:			to send IDC command immediately
*	9. IDC_CMD_SCHEDULE_EVENT:		to schedule events
*	10. IDC_CMD_SCHEDULE_START:		to run schedule
*	11. IDC_CMD_STOP_EVENT:			to stop specific event from specific schedule
*	12. IDC_CMD_PURGE:			to clean IDC RX FIFO
*	13. IDC_CMD_GET_SCHEDULE_STATUS:	to get status of schedule (bitmap, busy:1, idle:0)
*	14. IDC_CMD_CHECK_EVENT_SEND_OUT:	to check all events are sent out from IDC or not
*	15. IDC_CMD_SET_PIN_CONFIG:		to set pinmux beteen internal and external pins (K2, Denali Only)
*	16. IDC_CMD_GET_PIN_CONFIG:		to get pinumx config (K2, Denali Only)
*
...
DCL_STATUS DclIDC_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch(cmd)
	{
		...
		case IDC_CMD_SEND_EVENT:
		{
			...
			drv_idc_send_event(pr_ctrl_schedule_event->num_event, (IDC_EVENT_T *) &(pr_ctrl_schedule_event->schedule_event));
			return_value = STATUS_OK;
			break;
		}
