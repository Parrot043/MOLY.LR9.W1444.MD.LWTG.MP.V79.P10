...
static void cc_irq_msg_write_hdr(cc_irq_msg_pkt_hdr_t* hdr, kal_uint32 type, kal_uint32 pkt_num, kal_uint32 pkt_len, kal_uint32 seq)
{
    ...
    *(kal_uint32*)pkt_ptr = CC_IRQ_MSG_MAGIC_NUMBER;
    ...
    if (payload_remain_len > 0)
    {
        ...
        if (payload_remain_len > CC_IRQ_MSG_1ST_PKT_LP_MAX_LEN)
        {
            ...
            CC_IRQ_MEM_CPY((void*)pkt_ptr, (void*)_local_para_ptr, CC_IRQ_MSG_1ST_PKT_LP_MAX_LEN);
        ...
        else
        {
            ...
            CC_IRQ_MEM_CPY((void*)pkt_ptr, (void*)_local_para_ptr, payload_remain_len);
...
extern kal_uint32 cc_irq_msg_ut_pkt_enqueue(void *ptr, kal_uint32 length);
        ...
        {
            ret = cc_irq_sys_tx_with_buffer((void*)lp_ptr, pkt_len);
            if (ret != CC_IRQ_SUCCESS)
                return ret;
        }
#else        
    ...
    CC_IRQ_FREE_BUFFER(_local_para_ptr);
...
void cc_irq_msg_isr(void *pkt, kal_uint32 len)
{
    ...
    if (hdr->type == CC_IRQ_SYS_PKT_TYPE_SINGLE || hdr->type == CC_IRQ_SYS_PKT_TYPE_HEAD)
    {
        ...
        if (pkt_len > sizeof(ilm_struct))
        {
            ...
            CC_IRQ_MEM_CPY((void *)int_lp_ptr, (void *)lp_ptr, pkt_len - sizeof(ilm_struct));
        }
...
extern void cc_irq_ut_msg_isr(kal_uint32 dst_mod, kal_uint32 sap, kal_uint32 msg, local_para_struct *lp);
    ...
    else if (hdr->type == CC_IRQ_SYS_PKT_TYPE_BODY || hdr->type == CC_IRQ_SYS_PKT_TYPE_TAIL)
    {
        ...
        CC_IRQ_MEM_CPY((void *)cc_irq_msg_rx_socket[i].rx_ptr + cc_irq_msg_rx_socket[i].rx_len, (void *)pkt, pkt_len);
