 ...
 * Data Declaration                                                          *
 *****************************************************************************/
CC_IRQ_INTSRAM_ZI static kal_uint32 cc_irq_test_case;
...
CC_IRQ_INTSRAM_ZI static kal_uint32 cc_irq_it_seq[CC_IRQ_PARA_NUMBER];
...
CC_IRQ_INTSRAM_ZI static kal_uint32 tx_seq;
...
extern kal_uint32 LockIRQ();    // for cache pollute
extern void RestoreIRQ(kal_uint32);    // for cache pollute
...
enum {
    ...
    CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN1,
    CC_IRQ_UT_DIR2_MSG_TIME_PROFILE_LEN1,
    CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN2,
    CC_IRQ_UT_DIR2_MSG_TIME_PROFILE_LEN2,    
#endif
#if defined(__CC_IRQ_ON_PCCIF__)
...
#define CC_IRQ_MSG_UT_VAR_LEN_MAX       (1024 - 4)
...
kal_uint32 cc_irq_ut_msg_profile_rx_index = 0, cc_irq_ut_msg_profile_tx_index = 0;
...
dbg_enter_cnt++;        	
        ...
        break;
#if defined(__CORE1__)
            case CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN1 || CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN2:
#else
            case CC_IRQ_UT_DIR2_MSG_TIME_PROFILE_LEN1 || CC_IRQ_UT_DIR2_MSG_TIME_PROFILE_LEN2:
#endif
        {
            msg_time_profile_proc_rx_return_from_tx_ilm = ust_get_current_time();
            test_done = 1;
        }
        break;        
    default:
...
void cc_irq_msg_var_len_tx(kal_uint32 payload_type)
{
    ...
    for (tx_seq = 0; tx_seq <= CC_IRQ_MSG_UT_VAR_LEN_MAX - sizeof(local_para_struct); tx_seq++)
    {
        ...
        cc_irq_msg_send5(tx_seq & 0xFFFF, 0xFFFF - tx_seq, 0xAA00 | (tx_seq % 0x100), 0x5500 | (tx_seq >> 8), (local_para_struct*)test_ptr);
    }
}
...
void cc_irq_msg_con_skt_tx()
{
    ...
    for (i = 0; i < CC_IRQ_MSG_UT_TX_SKT_NUM; i++)
    {
        ...
        cc_irq_msg_send5(0x5555, 0xAAAA, 0xFFFF, 0x0000, msg);
    ...
    for (i = 0; i < pkt_per_msg; i++)
    {
        for (j = 0; j < CC_IRQ_MSG_UT_TX_SKT_NUM; j++)
        {
            ...
            cc_irq_sys_tx_with_buffer((void*)cc_irq_msg_ut_pkt_que[pkt_index], cc_irq_msg_ut_pkt_len[pkt_index]);
        }
    }
}
...
{
    msg_time_profile_proc_rx_lisr = ust_get_current_time();
}
...
kal_uint32 cc_irq_profile_cache_buffer[(64 * 1024) / 4];

void cc_irq_msg_time_profile_rx(kal_uint32 index)
{
    kal_uint32 duration;
    kal_uint32 profile_iter;

    for (profile_iter = 0; profile_iter < CC_IRQ_MSG_TIME_PROFILE_ITER; profile_iter++) 
    {
        test_done = 0;
extern void invalidate_icache();        
        CACHE_POLLUTE(4, cc_irq_profile_cache_buffer);
        invalidate_icache();
        cc_irq_unmask(CC_IRQ_MSG_RX_INDEX);
#if defined(__CORE1__)        
        SHARED_VAR(cc_irq_share_var).dir2_tx_notify = 1;
        ...
        duration = ust_us_duration(msg_time_profile_proc_rx_lisr, msg_time_profile_proc_rx_return_from_tx_ilm);
        if (duration > msg_time_record_rx_max[index])
            msg_time_record_rx_max[index] = duration;
    }
}

void cc_irq_msg_time_profile_tx(kal_uint32 lp_length, kal_uint32 log_index)
{
    kal_uint32 profile_iter, duration;
    kal_int32  ret;
    local_para_struct *test_ptr;

    test_ptr = (local_para_struct *)construct_local_para(sizeof(local_para_struct) + lp_length, 0);
    
    for (profile_iter = 0; profile_iter < CC_IRQ_MSG_TIME_PROFILE_ITER; profile_iter++)
    {
        // wait RX ready to receive.
#if defined(__CORE1__)
        while(SHARED_VAR(cc_irq_share_var).dir1_tx_notify == 0);
        SHARED_VAR(cc_irq_share_var).dir1_tx_notify = 0;
#else
...
extern void invalidate_icache();
        ...
        invalidate_icache();
        // log start time
        msg_time_profile_proc_start_tx = ust_get_current_time();
        // cc irq msg send
        ret = cc_irq_msg_send5(1, 1, 1, 1, test_ptr);
        // log end time
        msg_time_profile_proc_return_from_tx = ust_get_current_time();

        ASSERT(ret == CC_IRQ_SUCCESS);
        
        // record worst case
        duration = ust_us_duration(msg_time_profile_proc_start_tx, msg_time_profile_proc_return_from_tx);
        if (duration > msg_time_record_tx_max[log_index])
            msg_time_record_tx_max[log_index] = duration;
    }
}
...
extern void cc_sys_comm_init();
    ...
    cc_irq_test_case = CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN1;
    test_item_init(cc_irq_test_case, CC_IRQ_NO_UNMASK_IRQ);
#if defined(__CORE1__)
    ...
    cc_irq_test_case = CC_IRQ_UT_DIR2_MSG_TIME_PROFILE_LEN1;
    test_item_init(cc_irq_test_case, CC_IRQ_NO_UNMASK_IRQ);
#if defined(__CORE1__)
        cc_irq_msg_time_profile_tx(128, 0);
#else
    ...
    cc_irq_test_case = CC_IRQ_UT_DIR1_MSG_TIME_PROFILE_LEN2;
    test_item_init(cc_irq_test_case, CC_IRQ_NO_UNMASK_IRQ);
#if defined(__CORE1__)
...
void cc_irq_misc_ut()
{
    ...
    /* case 1 */
    cc_irq_test_case = CC_IRQ_UT_DIR1_USER_BASIC;
    test_item_init(cc_irq_test_case, CC_IRQ_UNMASK_IRQ);
#if defined(__CORE1__)    
    cc_irq_user_basic_rx();
#else
    cc_irq_user_basic_tx();
#endif
    test_item_deinit(cc_irq_test_case);

    /* case 2 */
    cc_irq_test_case = CC_IRQ_UT_DIR2_USER_BASIC;
    test_item_init(cc_irq_test_case, CC_IRQ_UNMASK_IRQ);
#if defined(__CORE1__)    
    cc_irq_user_basic_tx();
#else
