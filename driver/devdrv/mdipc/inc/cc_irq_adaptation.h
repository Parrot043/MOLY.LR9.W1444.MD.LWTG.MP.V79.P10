...
/* ==== MD PCCIF function ==== */
#define CC_IRQ_DRV_INIT()                                  md_pccif_local_hw_init()
...
#define CC_IRQ_DRV_ISR(index)                              md_pccif_isr(index)
/* ==== Interrupt Driver Adaptation ==== */
#define CC_IRQ_DRV_MASK(irq_number)                        IRQMask(irq_number)
#define CC_IRQ_DRV_UNMASK(irq_number)                      IRQUmask(irq_number)
#define CC_IRQ_DRV_REG_ISR(irq_number)                     IRQ_Register_LISR(irq_number, cc_irq_lisr, "CC_IRQ")
...
/* ==== Platform-dependent HW function */
#define CC_IRQ_DMB()
...
#define CC_IRQ_MEM_CPY(dst, src, len)                   kal_mem_cpy(dst, src, len)
#define CC_IRQ_FREE_BUFFER(buffer_addr)                 free_local_para(buffer_addr)
#define CC_IRQ_SYS_GET_EX_LOG_ADDR()                    md_pccif_get_ctrl_sram_addr()
#define CC_IRQ_GET_VERSION()                            md_pccif_get_version()

#define CC_IRQ_OPTION_IS_ENABLED()                      gmss_is_cdma2000_supported()
...
#define CC_IRQ_SET_SENSITIVITY_LEVEL(index)                IRQSensitivity((index), LEVEL_SENSITIVE)
...
#define CC_IRQ_MSG_MOD_ID_IN_TX_RANGE(mod_id)          (1)
#else
...
#if defined(__MD1__)
#define CC_IRQ_MSG_MOD_ID_IN_TX_RANGE(module_id)           ((IS_GLOBAL_MD2_MOD(module_id)) || (IS_GLOBAL_MD3_MOD(module_id)))
#else
#error "undefined behavior for MD2"
#endif

#endif    /* defined(__CC_IRQ_UT__) */
