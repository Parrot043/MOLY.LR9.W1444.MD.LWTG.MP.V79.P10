 ...
 * removed!
 * removed!
 * 	diable mdinfra bus monitor clock  and take off debub code.
 *
 * removed!
 * removed!
 ...
 * removed!
 * removed!
 * MP7 bus monitor debug support for EMI MPU violation.
 *
 * removed!
 * removed!
...
void drv_mdmcu_busmon_lisr()
{

   //for debug, force assert
   EXT_ASSERT(0,61,0,0);
...
void drv_mdperi_busmon_set_trg_cb(busmon_intr_cb callback)
{
   ...
   //DRV_Register_HISR(DRV_BUSMON1_HISR_ID, drv_mdperi_busmon_hisr);
