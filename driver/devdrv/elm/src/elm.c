...
ELM_INTERNAL ELM_GENERNAL_PROFILE_T elm_profile_debug_latency; //for 60 qbit debug check
ELM_INTERNAL kal_uint32 elm_debug_excute = 0; //high priority hisr won't excute
...
#define ELM_CODE_INTERNAL __attribute__ ((section ("INTSRAM_ROCODE")))
#else
#define ELM_CODE_INTERNAL
#endif

ELM_CODE_INTERNAL void _ELM_debug_start(void)
{
#ifdef __ELM_RUNTIME_PROFILE__	
#if defined(__UNIFIED_ISR_LEVEL__)
	//avoid encour 2/3/4g frametick ELM profile
	if (!kal_if_hisr())
	{
		elm_debug_excute = 1; //execute
	}
	else
	{
		if(kal_get_current_task_priority() > 60)
		{
			elm_debug_excute = 1; //execute
		}
		else
		{
			elm_debug_excute = 0; //not execute
		}
	}
...
#endif
#endif //#ifdef __ELM_RUNTIME_PROFILE__
}

ELM_CODE_INTERNAL void _ELM_debug_stop(void)
{
#ifdef __ELM_RUNTIME_PROFILE__
#if defined(__UNIFIED_ISR_LEVEL__)
	if(elm_debug_excute)
...
ELM_CODE_INTERNAL void _ELM_debug_dump(void)
{
#ifdef __ELM_RUNTIME_PROFILE__
#if defined(__UNIFIED_ISR_LEVEL__)
	if(elm_debug_excute)
	{
		...
		ELM_GET_CUR_CNT(ELM_WR, ELM_TYPE_TRANS, &elm_profile_debug_latency.w_trans);
		ELM_GET_CUR_CNT(ELM_WR, ELM_TYPE_LATENCY, &elm_profile_debug_latency.avg_wlat);
		if(elm_profile_debug_latency.w_trans != 0)
		{
			elm_profile_debug_latency.avg_wlat = elm_profile_debug_latency.avg_wlat / elm_profile_debug_latency.w_trans;
		}
		ELM_GET_CUR_CNT(ELM_RD, ELM_TYPE_TRANS, &elm_profile_debug_latency.r_trans);
		ELM_GET_CUR_CNT(ELM_RD, ELM_TYPE_LATENCY, &elm_profile_debug_latency.avg_rlat);
		if(elm_profile_debug_latency.r_trans != 0)
		{
			elm_profile_debug_latency.avg_rlat =  elm_profile_debug_latency.avg_rlat / elm_profile_debug_latency.r_trans;
		}		
	}
#endif
#endif //#ifdef __ELM_RUNTIME_PROFILE__
