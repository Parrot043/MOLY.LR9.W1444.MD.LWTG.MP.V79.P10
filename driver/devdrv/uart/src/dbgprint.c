 ...
 * removed!
 * removed!
 * update SLT UART log port configure.
 *
 * removed!
 * removed!
 * update:
...
#if (defined(MT6290) || defined(MT6595) || ((defined(MT6752)||defined(MT6735))&&defined(__MD1__))) && defined(__ARM7EJ_S__)
...
extern void UART_IOPAD_Configure(UART_PORT dev);
extern void UART_DriverInit(UART_PORT port);
extern kal_bool U_Open(UART_PORT port, module_type ownerid);
...
void dbg_print(char *fmt,...)
{
   ...
   DCL_HANDLE handle;
	...
	if(dbg_init_flag!=KAL_TRUE){
		...
		UART_IOPAD_Configure(DBG_PRINT_PORT);
		// Bind COM port
		handle = DclSerialPort_Open(DBG_PRINT_PORT, 0);
		if(handle == STATUS_INVALID_DEVICE){
			ASSERT(0);
		}
		UART_DriverInit(DBG_PRINT_PORT);
		status = U_Open(DBG_PRINT_PORT, MOD_DRV_DBG);
		if (status != KAL_TRUE) {
		...
		U_SetDCBConfig(DBG_PRINT_PORT, (UARTDCBStruct *)(&(data.rUARTConfig)), (module_type)data.u4OwenrId );
	}
...
void UART_IOPAD_Configure_SLT(UART_PORT dev){

#if defined(MT6735)
		kal_uint32 gpio_val;
		// APB Module gpio
		#define GPIO_BASE_SLT (0xA0211000)
			//set io mode : GPIO76,GPIO77 => MDUART mode
			gpio_val = DRV_Reg32((GPIO_BASE_SLT+0x370));
			gpio_val &= ~(0x3F<<19);
			gpio_val |= (0x1B<<19);
			DRV_WriteReg32((GPIO_BASE_SLT+0x370), gpio_val);
			
			gpio_val = DRV_Reg32((GPIO_BASE_SLT+0xA30));  //Set URXD PullEn
			gpio_val |= (0x1<<15);
			DRV_WriteReg32((GPIO_BASE_SLT+0xA30), gpio_val);

			gpio_val = DRV_Reg32((GPIO_BASE_SLT+0xA50));  //Set URXD PullSel
			gpio_val |= (0x1<<15);
			DRV_WriteReg32((GPIO_BASE_SLT+0xA50), gpio_val);		
#else
	UART_IOPAD_Configure(dev);
#endif
...
static void dbg_uart_init(void){
	...
	kal_uint32 uart_base = UART_BaseAddr[DBG_PRINT_PORT];
	UART_IOPAD_Configure_SLT(DBG_PRINT_PORT);
