 ...
 *******************************************************************************/
void boot_init_spi_clock()
{
    #if defined(MT6290)
    #if !defined(__SERIAL_FLASH_SUPPORT__) //SPI project will not excute CG operation
    /* Router project */
     ...
     */
    #else
    PDN_SET(PDN_SPI);// bus clock off
    IPCG_SET(IPCG_SPI);//SPI working clock off
    #endif//end of #if defined(__SMART_PHONE_MODEM__) && !(defined(__X_BOOTING__))
    #endif//end of #if !defined(__SERIAL_FLASH__)
