...
SIM_ICC_APPLICATION  sim_get_app_from_logicalNum(kal_uint32 logicalNum)
{
	kal_uint32 loopIndex, loopMax = iccSlotNum;
...
#if defined(SIM_DRV_SWITCH_MT6306)
void sim3_hot_plug_eint_cb_rm_h(void)
{
	kal_uint32 loopIndex;
	for(loopIndex = 0; loopIndex < iccSlotNum; loopIndex++)
	{
		if(SIM_ICC_APPLICATION_PHONE3 == iccHotPlugTable[loopIndex].application)
		{
			break;
		}
	}
	...
	{
		sim_hot_plug_eint_cb_rm_h_common(loopIndex, SIM_ICC_APPLICATION_PHONE3);   
	}

	EINT_Set_Polarity(iccHotPlugTable[loopIndex].eintNo, iccHotPlugTable[loopIndex].polarity);
}

void sim3_hot_plug_eint_cb_rm_l(void)
{
	kal_uint32 loopIndex;
	for(loopIndex = 0; loopIndex < iccSlotNum; loopIndex++)
	{
	...
	{
		sim_hot_plug_eint_cb_rm_l_common(loopIndex, SIM_ICC_APPLICATION_PHONE3);   
	}
                           ...
                           kal_uint32 socketType)
{
	...
	{
		if ((kal_bool)polarity == KAL_TRUE)
			EINT_Registration(eintNo, KAL_TRUE, (kal_bool)polarity, sim3_hot_plug_eint_cb_rm_h, KAL_TRUE);
		else
			EINT_Registration(eintNo, KAL_TRUE, (kal_bool)polarity, sim3_hot_plug_eint_cb_rm_l, KAL_TRUE);
	}	
#endif	
}
#endif
