...
#if defined(__NVRAM_SECRET_DATA__) || defined(__NVRAM_KEY_RUNTIME_GEN__)
...
void custom_nvram_encrypt(unsigned char const *keys, unsigned char *buffer, unsigned int length, int scrambleIndex)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
   #if !defined(_SECURE_RO_ENABLE__)&& defined(__NVRAM_KEY_RUNTIME_GEN__)
     kal_int32 ret;
   kal_uint8 key_buffer[256];
    ret=SST_Get_NVRAM_SW_Key(key_buffer,256,NVRAM_CUSTOM_KEY_SEED,NVRAM_CUSTOM_KEY_SEED_SIZE);
