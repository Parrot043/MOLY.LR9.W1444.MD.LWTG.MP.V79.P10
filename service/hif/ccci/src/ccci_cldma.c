 ...
 * Description:
 * ------------
 *   This file handles the CCCI functions that are CLDMA specified (ex. share memory initialization and boot up handshake)
 *
 * Author:
 * -------
 * -------
 ...
 * removed!
 * removed!
 * [CCCI] Sync MT6290 CCCI HS1 ex_set_memorydump_flag mechanism to SOP chip.
 *
 * removed!
 * removed!
 * [CCCI] Sync MT6290 CCCI HS1 ex_set_memorydump_flag mechanism to SOP chip.
 ...
 * removed!
 * removed!
 * [CCCI] Change include file emi.h to emi_if.h
 *
 * removed!
 * removed!
 * [CCCI] Change router compile option and get share memory offset from API
 *
 * removed!
 * removed!
...
#endif
#include "drv_comm.h"
#if defined(__PLATFORM_ROUTER__)
#include "emi_if.h"
#include "cache_sw.h"
#endif

//#include "ccci_cldma.h"

#if defined(__MTK_TARGET__)
#include "init_public.h" //for meta mode
//    #if defined(ATEST_SYS_ENABLE) || defined(MT6290)|| !defined(__HIF_CLDMA_SUPPORT__)
    #if defined(ATEST_SYS_ENABLE) || !defined(__HIF_CLDMA_SUPPORT__)
...
// The reason + QBM_HEAD_SIZE is for ROME E1, HIF needs to record this buff whether is cacheable or non-cacheable
...
{
#if defined(__PLATFORM_ROUTER__)
    // Query offset from MD EMI provided API, current owner is SD3
    drv_ramc_dram_info_t drv_ramc_dram_info;
	drv_ramc_get_dram_info(&drv_ramc_dram_info);
   
	*ShareMemBase += drv_ramc_dram_info.apmcu_ofst;
	*ShareMemBase = virt_to_phys((void *)*ShareMemBase);
#endif
}
		...
		if ((size >> i) == 1) {
		    // refer the mapping table in mpu.h
                    ...
                    if(size&(size-1)){
                        ...
                        CCIF_SHARE_MEM_MPU_ENABLE(addr, i);
                    }
...
/*!
 *  @brief     ccci_cldma_hs1: notify AP that MD need the run time data for information check.
 *  @return   void
 */
...
//    kal_uint8           ccci_hs_buff[CCCI_POLLING_MODE_BUF_SZ]; 
    ...
    while (CCCI_HANDSHAKE1_AP_ACK != get_ccci_boot_state()){
        ...
        hif_ret = hifcldma_simple_receive_gpd(0, p_gpd, CCCI_HS_POLLING_TIMEOUT);
        if(KAL_TRUE == hif_ret){
            ...
            if((rcv_size - sizeof(CCCI_BUFF_T)) != sizeof(MODEM_RUNTIME_DATA)){
                EXT_ASSERT(KAL_FALSE, rcv_size - sizeof(CCCI_BUFF_T), sizeof(MODEM_RUNTIME_DATA),0);
            }
...
#if defined(__PLATFORM_ROUTER__)// SP doesn't need remap & MPU protection
...
/*!
 *  @brief     ccci_cldma_hs2: AP_MD handshake part II
