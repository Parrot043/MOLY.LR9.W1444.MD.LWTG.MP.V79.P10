 ...
 *
 *************************************************************************/
kal_uint32 remap_module_id_from_global_to_local(kal_uint32 global_module)
{
	int k;

	ASSERT(global_module < MOD_ID_GLOBAL_TAIL);
 ...
 *
 *************************************************************************/
kal_uint32 remap_module_id_from_local_to_global(kal_uint32 local_module)
{
	int k;
	
	ASSERT(local_module > MOD_ID_GLOBAL_TAIL);
	
	for(k=1; k<(sizeof(module_remap_array)/sizeof(module_remap_struct)); k++)
	{
		if(module_remap_array[k].local_module == local_module)
		{
			return module_remap_array[k].global_module;
		}
	}
