...
void ApFactoryProceture()
{
    ...
    if(status == AP_FACTORY_ENTER_USB_DL){
        ...
        //For router & super dongle, USB is controlled by AP
        //Set the channel type to CCCI 
        g_cs_bl_param->sync_chl_type = CHL_CCCI;
...
kal_uint32 LoadAndRunApBootloader()
{
    ...
    if(ApBlShareInfoInit(ap_bl_addr) != KAL_TRUE){
        BL_PRINT(LOG_ERROR, "Fail to initializa parameters shared with AP\n\r");
        return INVALID_START_ADDR;       
    }

    *(volatile unsigned *)ap_bl_addr = 0x0;
    //set AP jump instruction to base address + 0x1000
    *(volatile unsigned *)(ap_bl_addr+0xC) = ap_bl_addr + 0x1000 | 0x1;
    *((volatile unsigned *)0x0) = 0x000CF240;
    *((volatile unsigned *)0x4) = 0x4080F2C0;
    *((volatile unsigned *)0x8) = 0x47086801;
    ...
    *(volatile unsigned *)(ap_bl_addr+0x4) = 0x24542454;
    ...
    //Use the last EMI MPU region to protect MD DRAM region
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 0, EMI_MPU_NOPORTECT);
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 1, EMI_MPU_NOPORTECT);
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 2, EMI_MPU_SR_NSNA);
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 3, EMI_MPU_NOACCESS);
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 4, EMI_MPU_NOPORTECT);
    EMI_MPU_Set_Region_APC(EMI_MPU_REGION_CP, 5, EMI_MPU_NOPORTECT);    
    EMI_MPU_Set_Region_Address(EMI_MPU_REGION_CP, 0, MD_SIZE);
    EMI_MPU_Lock();
...
#endif /*__USB_DOWNLOAD__ || __UART_DOWNLOAD__*/
    {
        ...
        //wait AP jump to start address
        while (0x62906290 != *((volatile unsigned *)ap_bl_addr));
