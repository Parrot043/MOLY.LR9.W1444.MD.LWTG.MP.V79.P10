...
static const char * TS_GetTsidName(TSID tsid)
{
    switch(tsid) {
        ...
        BR_ENUM_TO_CASE_STRING(TSID_BL_LOAD_AP_BOOTLOADER)
        ...
        BR_ENUM_TO_CASE_STRING(TSID_BL_SDIO_SYNC_BEFORE_PLL)
        BR_ENUM_TO_CASE_STRING(TSID_BL_SET_PLL)
        BR_ENUM_TO_CASE_STRING(TSID_BL_SDIO_SYNC_AFTER_PLL)
        BR_ENUM_TO_CASE_STRING(TSID_BL_SET_IO)
        BR_ENUM_TO_CASE_STRING(TSID_BL_SET_PMIC)
        BR_ENUM_TO_CASE_STRING(TSID_BL_SET_HW_OWN)
        ...
        BR_ENUM_TO_CASE_STRING(TSID_BL_UART_START_CMD)
        ...
        BR_ENUM_TO_CASE_STRING(TSID_BL_INIT_MINI_FLASH_DRV_FOR_AP)
        BR_ENUM_TO_CASE_STRING(TSID_BL_LOAD_AP_BOOTLOADER_BODY)
...
void TS_DumpTimeStamps(void)
{
    ...
    for(i=0; i<TSID_NUM; i++)
    {
        ...
        {
            BL_PRINT(LOG_INFO, "BL> --------------------------------\r\n");
        }
        
        lastTSvalue = p->m_begin_tick;

        BL_PRINT(LOG_INFO, "BL> %s, dur=%d, (s=%d, e=%d)", TS_GetTsidName(p->m_id), p->m_duration, p->m_begin_tick, p->m_end_tick);
        ...
        switch(p->m_id)
        {
            ...
            case TSID_BL_LOAD_MAUI_BODY:
                BL_PRINT(LOG_INFO, ", size=%d", p->m_info2);
                BL_PRINT(LOG_INFO, ", throughput=%d KB/s", (U32)(p->m_info2*(1000.0/1024)/p->m_duration));
                break;
