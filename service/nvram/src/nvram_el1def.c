...
void nvram_get_eL1_default_value_to_write(nvram_lid_core_enum lid, kal_uint8 *buffer, kal_uint16 buffer_size)
{
	...
	{
		kal_mem_cpy(buffer, 
         LTE_EtVinLutTbl[lid-NVRAM_EF_EL1_ETVINLUTDATA_1STBAND_LID+1],
	...
	{
		kal_mem_cpy(buffer, &(LTE_EtCompParam), sizeof(LTE_RfcEtCompParam_T));
	}
	break;
  ...
  case NVRAM_EF_EL1_GPS_CO_TMS_DATA_LID:
  {
	  kal_mem_cpy(buffer, LTE_FreqAdjGPScotmsTbl[0], NVRAM_EF_EL1_GPS_CO_TMS_DATA_SIZE);
  }
  break;
  ...
  case NVRAM_EF_EL1_SVLTE_BAND_INDICATOR_LID:
  {
     kal_mem_cpy(buffer, lteBandIndArraySVLTE, NVRAM_EF_EL1_SVLTE_BAND_INDICATOR_SIZE);
  }
  break;
#endif
