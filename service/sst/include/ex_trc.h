...
BEGIN_TRACE_MAP(MOD_EX_LOG)
    ...
    TRC_MSG(EX_LOG_TYPE_FATALERR_CC_C2K, "Exception type: C2K invoked")
    ...
    /* cp15 registers*/
    TRC_MSG(EX_LOG_CP15_REG_IFSR, "IFSR = 0x%08X")
    TRC_MSG(EX_LOG_CP15_REG_DFSR, "DFSR = 0x%08X")
    TRC_MSG(EX_LOG_CP15_REG_FAR, "FAR = 0x%08X")
    TRC_MSG(EX_LOG_CP15_REG_IFAR, "IFAR = 0x%08X")
    ...
    TRC_MSG(MDDBG_API_UT_ENABLE, "mddbg_enable(%d) : %d")
    ...
    TRC_MSG(MDDBG_API_UT_SET_BP_PARAM, "ctrl_addr = 0x%08X, ctrl_value = 0x%08X, found_idx = %d, found_is_set = %d, not_used_idx = %d, set_idx = %d")
