...
** Constants
*******************************************************************************/
#define EL1_TARGET_MAX_SUPPORT_BAND_NUM            14
...
#if defined(__ADD_NEW_LTE_RF_SUPPORTED_BAND__)
#define LTE_SPEC_SUPPORT_BAND_NUM                  27
#else
#define LTE_SPEC_SUPPORT_BAND_NUM                  26
#endif
...
#define LTE_BAND_ET_BW_SWITCH_EVENT(b)           b##_ET_BW_SWITCH_EVENT
#define LTE_ET_BW_SWITCH_EVENT(b)                LTE_BAND_ET_BW_SWITCH_EVENT(b)

#define LTE_BAND_ET_BW_SWITCH_DATA(b)            b##_ET_BW_SWITCH_DATA
#define LTE_ET_BW_SWITCH_DATA(b)                 LTE_BAND_ET_BW_SWITCH_DATA(b)

#define LTE_BAND_ET_BW_SWITCH_SECTION_DATA(b)    b##_ET_BW_SWITCH_SECTION_DATA
#define LTE_ET_BW_SWITCH_SECTION_DATA(b)         LTE_BAND_ET_BW_SWITCH_SECTION_DATA(b)

#define LTE_BAND_ET_OUTER_LOOP_GAIN_SETTING(b)   b##_ET_OUTER_LOOP_GAIN_SETTING
#define LTE_ET_OUTER_LOOP_GAIN_SETTING(b)        LTE_BAND_ET_OUTER_LOOP_GAIN_SETTING(b)

#define LTE_BAND_ET_VPA_OFFSET_VOLTAGE_SETTING(b)   b##_ET_VPA_OFFSET_VOLTAGE_SETTING
#define LTE_ET_VPA_OFFSET_VOLTAGE_SETTING(b)        LTE_BAND_ET_VPA_OFFSET_VOLTAGE_SETTING(b)

#define LTE_BAND_ET_MINIMUM_OUTPUT_VOLTAGE_SETTING(b)   b##_ET_MINIMUM_OUTPUT_VOLTAGE_SETTING
#define LTE_ET_MINIMUM_OUTPUT_VOLTAGE_SETTING(b)        LTE_BAND_ET_MINIMUM_OUTPUT_VOLTAGE_SETTING(b)

#define LTE_BAND_ET_MAXIMUM_OUTPUT_VOLTAGE_SETTING(b)   b##_ET_MAXIMUM_OUTPUT_VOLTAGE_SETTING
#define LTE_ET_MAXIMUM_OUTPUT_VOLTAGE_SETTING(b)        LTE_BAND_ET_MAXIMUM_OUTPUT_VOLTAGE_SETTING(b)
...
#define LTE_BAND_VPA_SOURCE_CONFIG(b)            b##_VPA_SOURCE
#define LTE_VPA_SOURCE_CONFIG(b)                 LTE_BAND_VPA_SOURCE_CONFIG(b)

/*===============================================================================*/

//Utility for RF customization
#define MICROSECOND_TO_26M(x)                      ((kal_int32)((x)*26))
#define MICROSECOND_TO_52M(x)                      ((kal_int32)((x)*52))
...
typedef enum
{
   ...
   VPA_SOURCE_VBAT_T    = VPA_SOURCE_HW_V_BATTERY,
...
#define LTE_RFC_ET_NUM_SUBBAND   (8)   /*number of subbands per band*/
#define LTE_RFC_ET_NUM_BAND      (14)  /*number of supported bands*/
/*move ET_CAL_PATH_DELAY_CAL_BY_SUBBAND to el1_meta_if.h*/
//#define ET_CAL_PATH_DELAY_CAL_BY_SUBBAND
/*to determine current status on DSP while in ET calibration*/
#define ET_CAL_DSP_STATUS_IDLE                ((kal_uint16)0x0000)
#define ET_CAL_DSP_STATUS_PATTERN_GEN_START   ((kal_uint16)0x0001)
#define ET_CAL_DSP_STATUS_PATTERN_GEN_DONE    ((kal_uint16)0x0002)

#ifdef __MTK_TARGET__
#define RF_ET_PARAM_SIZE   20480
#else
#define RF_ET_PARAM_SIZE   65535
#endif

typedef struct
{
   kal_uint16 VinVpaLut[LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
   kal_int16  VinPmLut [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
   kal_int16  VinAmLut [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
} LTE_RfcEtVinLutPerBand_T;

typedef struct
{
#ifdef ET_CAL_PATH_DELAY_CAL_BY_SUBBAND
   kal_int16  PathDelay             [LTE_RFC_ET_NUM_BAND][LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_CBW];
#else
   kal_int16  PathDelay             [LTE_RFC_ET_NUM_BAND][LTE_RFC_ET_NUM_CBW];
#endif
   kal_int16  Pout_max              [LTE_RFC_ET_NUM_BAND][LTE_RFC_ET_NUM_SUBBAND];                       /*14*8*/
   kal_int16  Pout_gain_offset      [LTE_RFC_ET_NUM_BAND][LTE_RFC_ET_NUM_SUBBAND];                       /*14*8*/
   kal_uint16 GainComp_0p5_dB       [LTE_RFC_ET_NUM_BAND];                                               /*14*/
   kal_uint16 GainComp_0p5_dB_delta [LTE_RFC_ET_NUM_BAND];                                               /*14*/
   kal_int16  PathDelayOffsetByTemp [LTE_RFC_ET_NUM_BAND][CAL_TEMP_SECTION_LTE][LTE_RFC_ET_NUM_CBW];     /*14*8*6*/
   kal_int16  GrpDlyOfstBySubband   [LTE_RFC_ET_NUM_BAND][LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_CBW];   /*14*8*6*/
   kal_uint16 VpaTargetStep2        [LTE_RFC_ET_NUM_BAND];                                               /*14*/
   kal_uint16 SmpValStep2Sec1;
   kal_int16  PaGainAdjustStep1     [LTE_RFC_ET_NUM_BAND];                                               /*14*/
   kal_uint32 PowThrdStep2          [LTE_RFC_ET_NUM_BAND];                                               /*14*/
} LTE_RfcEtCompParam_T;

typedef struct
{
   kal_uint16 LTE_ET_SUBFREQ_GROUP_PER_BAND  [LTE_RFC_ET_NUM_SUBBAND];
   kal_int16  LTE_ET_SUBBAND_RB_OFFSET       [LTE_RFC_ET_NUM_SUBBAND];
   kal_uint16 LTE_ET_RF_GEAR                 [LTE_RFC_ET_NUM_SUBBAND];
   kal_uint16 LTE_ET_BB_BACKOFF              [LTE_RFC_ET_NUM_SUBBAND];
   kal_uint16 Dmin32;
   kal_uint16 Grid_step;
   kal_uint16 Vreset;
#ifdef ET_CAL_PATH_DELAY_CAL_BY_SUBBAND
   kal_uint16 VpaScaleStep2                  [LTE_RFC_ET_NUM_SUBBAND];
#else
   kal_uint16 VpaScaleStep2;
   kal_uint16 SBIdxStep2;
#endif
   kal_uint16 SmpValStep2Sec1;
...
/* for normal Tx utilization */
typedef struct
{
   kal_uint16 VinVpaLut[LTE_RFC_ET_NUM_VIN];
   kal_int16  VinPmLut [LTE_RFC_ET_NUM_VIN];
   kal_int16  VinAmLut [LTE_RFC_ET_NUM_VIN];
   kal_int16  PathDelay[LTE_RFC_ET_NUM_CBW];
   kal_int16  Pout_max;
   kal_int16  Pout_gain_offset;
} LTE_RfcEtCalResultPerSubBand_T;

#ifdef ET_CAL_PATH_DELAY_CAL_BY_SUBBAND
/* for ET calibration utilization */
typedef struct
{
      kal_uint16 VinVpaLut       [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
      kal_int16  VinPmLut        [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
      kal_int16  VinAmLut        [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_VIN];
      kal_int16  PathDelay       [LTE_RFC_ET_NUM_SUBBAND][LTE_RFC_ET_NUM_CBW];
      kal_int16  Pout_max        [LTE_RFC_ET_NUM_SUBBAND];
      kal_int16  Pout_gain_offset[LTE_RFC_ET_NUM_SUBBAND];
...
typedef enum
{
   //0 is normal meta/1:enter cal mode/2:enter ET cal mode
...
/* pointer of DRAM table/variable, always access these table/variable through the pointer */
extern LTE_RfcEtVinLutPerBand_T* LTE_EtVinLutTbl[LTE_AUGMENTED_BAND_ARRAY_NUM];

/* body of DRAM table/variable */
...
extern LTE_Band lteBandIndArraySVLTE[LTE_AUGMENTED_BAND_ARRAY_NUM];
#endif
...
// LTE ET extern global variables.
extern kal_uint16 LTE_ET_ENABLE;
extern kal_uint16 LTE_ETM_chip_type;
extern kal_uint8  LTE_ET_MIPI_USID;
extern kal_uint8 ET_OUTER_LOOP_GAIN[ LTE_TARGET_MAX_SUPPORT_BAND_NUM ];
extern kal_uint16 ET_VPA_OFFSET_VOLTAGE[ LTE_TARGET_MAX_SUPPORT_BAND_NUM ];
extern kal_uint16 ET_MINIMUM_OUTPUT_VOLTAGE[ LTE_TARGET_MAX_SUPPORT_BAND_NUM ];
extern kal_uint16 ET_MAXIMUM_OUTPUT_VOLTAGE[ LTE_TARGET_MAX_SUPPORT_BAND_NUM ];
extern ERF_TEMPERATURE_SENSOR_CW_DATA_T erf_temperature_CW_data;
extern ERF_ETCAL_MODE_T EPHY_META_CAL_STATUS;

/* VPA Source */
extern LTE_VPA_SOURCE_CONFIGUATION_TYPE LTE_VpaSourceConfig[LTE_TARGET_MAX_SUPPORT_BAND_NUM];
