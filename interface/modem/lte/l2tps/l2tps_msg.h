...
enum
{
	...
	L2TPS_MSG_ID_DL_VOLTE_CALL_CTRL,
	L2TPS_MSG_ID_UL_VOLTE_CALL_CTRL_CNF
};
...
typedef struct
{
    ...
    uint8   rb_id;
    uint8   rb_idx;
    uint8   del_for_msg3_tx; //delete this DRB after scheduling n-th Msg3 HARQ Tx, n = del_for_msg3_tx, 0 means disable
} l2tps_add_rb_info_t;
...
typedef struct
{
    ...
    uint8   ra_mask;
    uint8   rsv0;
    uint8   drx_enable; //bool: TRUE = with DRX config
...
typedef struct
{
    uint32 volte_t_period;

}l2tps_cmd_dl_volte_call_ctrl;
