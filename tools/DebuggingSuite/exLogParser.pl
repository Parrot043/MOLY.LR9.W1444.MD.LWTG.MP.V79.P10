...
#* $Log$
#*
#* 03 04 2015 qmei.yang
#* [MOLY00097756] [SystemService][DebuggingSuite][Internal Refinement] Support MD3 exLogParsing
#* 	.
#*
#* 09 23 2014 qmei.yang
#* [MOLY00079441] [SystemService][DebuggingSuite][Request For Design Change] Support to parse ASSERT's load info
#* .
#*
...
elsif($exception_type[$ex_type] eq 'ASSERT_FAIL_EXCEPTION')
{
    $ex_log .= "  - load info = \n";
    $len = 63;
    $i = 0;
    while($i < $len) {
        $j = 0;
        while($j < 4) {
            $ex_log .= chr($ex_log_raw[($i * 4) + 0x100]);
            $ex_log .= chr($ex_log_raw[($i * 4) + 0x101]);
            $ex_log .= chr($ex_log_raw[($i * 4) + 0x102]);
            $ex_log .= chr($ex_log_raw[($i * 4) + 0x103]);
