...
ECHO [MT6595 Multi-Core Debugging]
...
START CVD.exe -Multi -Master -CORE=1 MT6595_MultiCore_AP.csf

:: ping 3 s
ping 127.0.0.1 -n 3 -w 1000 > nul

ECHO [Step 1] Press any key to start MD1 ^(Cortex-R4^) debugger...
START CVD.exe -Multi -Slave -CORE=2 MT6595_MultiCore_MD.csf
...
ECHO Starting AP ^(Cortex-A7^) debugger...
START CVD.exe -Multi -Master -CORE=1 MT6595_MultiCore_AP_Attach.csf

:: ping 3 s
PING 127.0.0.1 -n 3 -w 1000 > nul

ECHO Start MD ^(Cortex-R4^) debugger...
START CVD.exe -Multi -Slave -CORE=3 MT6595_MultiCore_MD_Attach.csf
...
ECHO start MD ^(Cortex-R4^) debugger...
START CVD.exe MT6595_MultiCore_MD.cmm
