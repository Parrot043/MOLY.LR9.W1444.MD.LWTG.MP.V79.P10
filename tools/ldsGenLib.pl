...
sub ldsGen_verno
{
    return " m0.57_MP7 + ldsFrame.pm ".&ldsFrame::ldsFrame_verno;
           # m0.57_MP7 , 20150323 by Carl, add smart logging feature for special denali project
           # m0.56_MP7 , 20150213 by Carl, Enlarge default ROME+ size  
           # m0.55_MP7 = m0.51_MP6 +  m0.52_MP6 +  m0.53_MP6 , 
           #             20150206 by Carl, add smart logging feature for special project
           # m0.54_MP7 , 20150122 by Carl, Reduce Denali LTG MD size from 56 MB to 48MB (MP7 only)
           # m0.53_MP7 , 20141225 by Carl, Reduce K2 MD size from 64 MB to 56MB (MP7 only)
           # m0.52 , 20141126 by Carl, Reduce MT6735 MD size from 64 MB to 56MB
           # m0.51 , 20141104 by Carl, Support MT6735 MD1
...
{
    my ($Setting_href) = @_;
    my ($nMaxSize, $nROMSize) = (0x5000000, 0xE00000); #80MB, 14MB
    $nMaxSize = 0x6000000 if(&FileInfo::find("TEST_LOAD_TYPE", "L1S")); # 96 MB
    $nMaxSize = 0x8000000 if(FileInfo::is("SMART_PHONE_CORE", "MODEM_ONLY")); #128MB
    $nMaxSize = 0x8000000 if(FileInfo::is("DHL_SET_LOG_BUF_SIZ", "65536"));  #128MB, increase size for special project    

    $nROMSize = GetPredefinedValue($Setting_href, "RAM_BASE", "ROM_LEN", $nROMSize);

    SetExistentValueByDefault($Setting_href, "ROM_LEN",  $nROMSize);
    SetExistentValueByDefault($Setting_href, "RAM_BASE", (sysUtil::GetNonCacheablePrefix($g_bb) | $nROMSize));
    SetExistentValueByDefault($Setting_href, "RAM_LEN",  ($nMaxSize - $nROMSize));    
...
{#smart phone doesn't use $g_nRamSize
    ...
    {
        # MD1: 90
        $nTotalSize = 0x3800000; # 56 MB (64->56 for slim)
...
sub MT6735_MemorySetting
{#smart phone doesn't use $g_nRamSize
    my ($Setting_href) = @_;
    my ($nTotalSize, $nROMSize) = (0, 0); # RamSize means MD available space

    $nTotalSize = 0x3800000; # 56 MB
    $nROMSize   =  0xD00000; # 13 MB

    # 48 MB for LTG MD
    $nTotalSize = 0x3000000 if(FileInfo::is("UMTS_MODE_SUPPORT", "UMTS_TDD128_MODE_SUPPORT"));

    # for MD ONLY LOAD, 128 MB
    $nTotalSize = 0x8000000 if(FileInfo::is("SMART_PHONE_CORE", "MODEM_ONLY"));

    # increase size for special project
    $nTotalSize = 0x7000000 if(FileInfo::is("DHL_SET_LOG_BUF_SIZ", "65536"));  #112MB
