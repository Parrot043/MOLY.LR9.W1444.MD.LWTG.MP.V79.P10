...
////    TRC_MSG(FUNC_SIM_CSIM_REQ_HANDLER, "sim_csim_req_handler()")      //mtk01612: __CSIM__    
    ...
    TRC_MSG(FUNC_SIM_RECOVERY_ENHANCEMENT_SWITCH_REQ_HANDLER, "sim_recovery_enhancement_switch_req_handler()")
    ...
    TRC_MSG(INFO_UIM_CAT_MSG,"uim_sat_proactive_command_ind_hdlr() - %Msat_proactive_cmd_enum")
    ...
    TRC_MSG(INFO_SVLTE_STK_ACTIVATE,"xTK switch - STK activate")
    TRC_MSG(INFO_SVLTE_UTK_ACTIVATE,"xTK switch - UTK activate")
    TRC_MSG(INFO_SVLTE_STK_READY,"sat_utk_enabled_update_req_handler() - sat_ready_to_start:%d")
    ...
    TRC_MSG(WARNING_MMDC_CS_MSG_NOT_ALLOW,"sim_sat_mmdc_cs_send_ilm() MSG NOT ALLOWED!")
    TRC_MSG(WARNING_UIM_CAT_MSG_NOT_ALLOW,"uim_sim_send_ilm() MSG NOT ALLOWED!")
    TRC_MSG(WARNING_UIM_CAT_UNEXPECTED_TR,"uim_sat_terminal_rsp_handler() - unexpected %Msat_proactive_cmd_enum")
    TRC_MSG(WARNING_UIM_CAT_UNEXPECTED_ENVELOPE,"uim_sat_envelope_handler() - unexpected %Msat_tag_enum")
    TRC_MSG(WARNING_REISSUE_REFRESH_WRONG_DATA,"sat_reissue_refresh_handler() WRONG DATA and INTERRUPT the REFRESH!")
