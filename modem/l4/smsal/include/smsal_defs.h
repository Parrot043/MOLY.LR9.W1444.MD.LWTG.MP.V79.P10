...
#include "protocol_section.h"  // define global_peer_msg_id
#include "dhl_trace.h"  // define dhl_peer_trace( )
...
#define SMSAL_MAX_PENDING_ACT    (8)  /* 1 for request from L4C to SMSAL,
...
#define SMSAL_DELIVER_PDU_MAX_LEN (163)	/*for deliver type*/

/* for CB queueing */
#define SMSAL_CB_ONE_PAGE_SIZE    (93)
#define SMSAL_CB_FLW_CTRL_MSG_NUM (5)
...
#define SMSAL_DHL_TRACE_PEER(W,X,Y,Z)   { \
    do {  \
        kal_uint8* p = NULL; \
        kal_uint16 l = 0; \
        if (X != NULL) \
            p = get_peer_buff_pdu(X, &l); \
        dhl_peer_trace(W, l, p, Y,Z); \
    } while (0); } 


#define SMSAL_ETWS_CHANNEL_START    (0x1100)
#define SMSAL_ETWS_CHANNEL_END      (0x1107)
#define SMSAL_CMAS_CHANNEL_START    (0x1112)
#define SMSAL_CMAS_CHANNEL_END      (0x112B)
