...
enum
{
        ...
        RMMI_CMUX_CHANNELS_END = (RMMI_CMUX_CHANNELS_START + (CMUX_VP_NUM_FOR_SINGLE_SIM*RMMI_MAX_SIM_NUM) + CMUX_VPEX_TOTAL_NUM - 1),
    #else
    ...
    #if defined(__SMART_PHONE_MODEM__)//(__AGPS_SUPPORT__)
        ...
        /* Above channels all have corresponding SRC id and PORT id */
        RMMI_SRC_END_CHANNEL = RMMI_USB2_CHANNEL,
        /* Below channels may not have PORT ID/SRC ID and have to assign SRC id manually if need */
        
    #if defined(__IMS_SUPPORT__) // ALPS01642620
        RMMI_IMSP_CHANNEL_START,
        RMMI_IMSP_CHANNEL_END = RMMI_IMSP_CHANNEL_START + 10, //MSP_MAX_TRANSACTION = 10, it must be the same with IMSP_MAX_TRANSACTIONs
    #endif
        RMMI_TST_CHANNEL,
        RMMI_TST_CHANNEL_END = RMMI_TST_CHANNEL + RMMI_MAX_SIM_NUM - 1,
...
typedef enum
{
   ...
   PH_FSIM_PUK_REQUIRED = 7, //Kinki: SIM-ME lock [MAUI_01264994]
    ...
    RMMI_ERR_LAST_PDN_DISC_NOT_ALLOWED = 151,
    ...
    RMMI_ERR_BT_SAP_CARD_ALREADY_POWERED_OFF = 614,
    RMMI_ERR_BT_SAP_CARD_ALREADY_POWERED_ON = 615,
    RMMI_ERR_CARD_TECHNICAL_PROBLEM = 616,

    RMMI_ERR_AT_NOT_ALLOWED_BY_CUSTOMER = 617
} rmmi_err_id_enum;
