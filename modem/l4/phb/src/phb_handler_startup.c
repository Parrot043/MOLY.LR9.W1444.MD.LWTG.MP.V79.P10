...
static phb_build_entry_struct phb_build_index_tbl_fdn[] =
{
#ifdef __PHB_USIM_SUPPORT__
        {
            FILE_U_FDN_IDX,
...
static void phb_build_adn(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    else
    {
        ...
        if (phb_ptr->phb_seek_done)
        {
            ...
            while ((seek_byte < (ADN_INDEX_MAX_ENTRIES_COUNT + 7) / 8) && (phb_ptr->phb_seek_adn_bitmap[seek_byte] & (1 << seek_bit)) == 0)
            {
                phb_data_desc_mark_free(
                            DATA_DESC_ADN,
                            cntx->pbr_rec,
...
void phb_startup_build_fdn_index_ex(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    kal_uint8 curr_sim = phb_which_sim_ex();
    kal_bool usim = phb_util_is_usim(curr_sim);
    ...
    while (phb_query_count[curr_sim] >=0 && phb_build_index_tbl[phb_query_count[curr_sim]].primary_ID)
    {
        ...
        phb_control_block_set(
            cblk,
            phb_build_index_tbl[phb_query_count[curr_sim]].child_cnf,
            phb_startup_build_fdn_index_ex,
            phb_startup_err_handler,
            startup_build_index);
...
void phb_startup_build_fdn_index(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    phb_build_index_tbl = &phb_build_index_tbl_fdn[0];
    phb_startup_build_fdn_index_ex(ilm_ptr, cblk);
}
