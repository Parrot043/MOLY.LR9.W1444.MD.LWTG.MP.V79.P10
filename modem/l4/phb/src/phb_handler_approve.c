...
void phb_approve_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
        ...
        phb_ptr->src_mod_id = ilm_ptr->src_mod_id;
        phb_ptr->dest_mod_id = ilm_ptr->dest_mod_id;
       ...
       */
        if ((phb_ptr->state == PHB_STATE_READY) && (control_block != NULL))
        {
            ...
            #ifdef __UMTS_TDD128_MODE__ /* special requested by CMCC TD test case: 12.2 */
...
#else
    phb_send_ilm2(phb_ptr->dest_mod_id, phb_ptr->src_mod_id, MSG_ID_L4CPHB_APPROVE_CNF, local_param_ptr, peer_buf_ptr);
#endif
...
void phb_update_nw_ecc_list_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
        ...
        ecc_src = (ecc_src_type_enum) (ECC_SRC_NW1 + phb_which_sim_ex());

        /* clear ecc from applib */
