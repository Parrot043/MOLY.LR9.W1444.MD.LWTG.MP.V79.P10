 ...
 * removed!
 * removed!
 * [CCMNI] IPv6 log 2-byte align bug fixed on MP7
 *
 * removed!
 * removed!
 ...
 * Merging
 * 	
 * 	//MOLY/BRANCHES_LR9/LR9.W1423.MD.LWTG.MP/mcu/middleware/hif/cccidev/ccci_ccmni/...
 * 	
 * 	to //MOLY/BRANCHES_LR9/LR9.W1444.MD.LWTG.MP/mcu/middleware/hif/cccidev/ccci_ccmni/...
 *
 * 06 27 2014 ap.wang
 * removed!
...
#define CCMNI_DUMP_ENABLE (0)
#define CCMNI_IPv6_DUMP (1)
...
#if CCMNI_IPv6_DUMP
static void ccmni_print_IPv6(kal_uint8* p_data, void* p_gpd, kal_uint8 p_protocal, kal_bool is_dl)
{
	kal_uint16 checksum16; //Transport layer check sum is all 2-Byte length
	kal_uint8* p_chk = &checksum16;
	kal_uint8* p_addr;
	kal_uint8 a_addr[16];
	kal_uint8* index = a_addr;
	kal_uint8 count = 0;
	kal_uint32* addr32 = (kal_uint32*)a_addr;	
	//kal_uint32 p_seq = *((kal_uint16*)(p_data + IPC_HDR_V6_HEADER_SIZE + 4)); // TCP
	//kal_uint32 p_ack = *((kal_uint16*)(p_data + IPC_HDR_V6_HEADER_SIZE + 8)); // TCP
	switch (p_protocal)	//differenet offset
	{
		case IPC_HDR_PROT_UDP :
			//checksum16 = *((kal_uint16*)(p_data + IPC_HDR_V6_HEADER_SIZE + 6));
			*p_chk = *(p_data + IPC_HDR_V6_HEADER_SIZE + 6 + 1);
			*(p_chk + 1) = *(p_data + IPC_HDR_V6_HEADER_SIZE + 6);
			break;
		case IPC_HDR_PROT_TCP :
			//checksum16 = *((kal_uint16*)(p_data + IPC_HDR_V6_HEADER_SIZE + 16));
			*p_chk = *(p_data + IPC_HDR_V6_HEADER_SIZE + 16 + 1);
			*(p_chk + 1) = *(p_data + IPC_HDR_V6_HEADER_SIZE + 16);
			break;
		case IPC_HDR_PROT_ICMP :
		case IPC_HDR_PROT_ICMPV6 :
			//checksum16 = *((kal_uint16*)(p_data + IPC_HDR_V6_HEADER_SIZE + 2));
			*p_chk = *(p_data + IPC_HDR_V6_HEADER_SIZE + 2 + 1);
			*(p_chk + 1) = *(p_data + IPC_HDR_V6_HEADER_SIZE + 2);
			break;
		default :
			checksum16 = 0;
			break;
	}
	//CCMNI_DL_DATA_IPv6_PACKET_TRACE			CCMNI 	   "[CCMNI DLv6] CCMNI%d: Protocal=%xl Checksum=%xl Src Addr=%xl"
	//checksum16 = (checksum16 >> 8) | (checksum16 << 8); // little endian to big endian
	if(is_dl){	//DL
		p_addr = p_data + IPC_HDR_V6_SRC_ADDR_OFFSET;
		//p_addr = (kal_uint32*)((int)(p_data + IPC_HDR_V6_SRC_ADDR_OFFSET) & (~0X3));	//Src addr
		//ccmni_data_trace(CCMNI_DL_DATA_IPv6_PACKET_TRACE, p_protocal, p_gpd, checksum16, p_addr[0],p_addr[1],p_addr[2], p_addr[3]);
	}
	else{	//UL
		p_addr = p_data + IPC_HDR_V6_DST_ADDR_OFFSET;
		//p_addr = (kal_uint32*)((int)(p_data + IPC_HDR_V6_DST_ADDR_OFFSET) & (~0X3)); //Dst addr
		//ccmni_data_trace(CCMNI_UL_DATA_IPv6_PACKET_TRACE, p_protocal, p_gpd, checksum16, p_addr[0],p_addr[1],p_addr[2], p_addr[3]);
	}

	for(count=0,index=a_addr;count<4;count++){
		index[0] = p_addr[3];
		index[1] = p_addr[2];
		index[2] = p_addr[1];
		index[3] = p_addr[0];
		index+=sizeof(kal_uint32);
		p_addr+=sizeof(kal_uint32);
	}
		
	if(is_dl){	//DL
		ccmni_data_trace(CCMNI_DL_DATA_IPv6_PACKET_TRACE, p_protocal, p_gpd, checksum16, addr32[0],addr32[1],addr32[2], addr32[3]);
	}
	else{	//UL
		ccmni_data_trace(CCMNI_UL_DATA_IPv6_PACKET_TRACE, p_protocal, p_gpd, checksum16, addr32[0],addr32[1],addr32[2], addr32[3]);
	}
        ...
        { //for ccmni GPD debug 
			...
			if(IPC_HDR_IS_V4(pdata)){
				ccmni_data_trace(CCMNI_DL_DATA_PACKET_TRACE,pDevice->dev_id, 
                             	current_gpd, (pdata[4]<<8|pdata[5]), (pdata[10]<<8|pdata[11]));
			}
#if CCMNI_IPv6_DUMP
			else if(IPC_HDR_IS_V6(pdata)){
				kal_uint8 pprotocal=pdata[6]; //next header
				ccmni_print_IPv6(pdata,current_gpd, pprotocal, 1);				
			}
#endif
			else{
				//not defined now
...
/*                    if(pdata->reserved != *seq_no){
                ...
                {//for trace IP_ID
					...
					pdata += sizeof(CCCI_BUFF_T);	//pass ccci header to get ip header 
					if(IPC_HDR_IS_V4(pdata)){
	                    ccmni_data_trace(CCMNI_UL_DATA_PACKET_TRACE,dev_id, 
                             	curr_gpd, (pdata[4]<<8|pdata[5]), (pdata[10]<<8|pdata[11]));
					}
#if CCMNI_IPv6_DUMP
					else if(IPC_HDR_IS_V6(pdata)){
						kal_uint8 pprotocal=pdata[6]; //next header
						ccmni_print_IPv6(pdata,curr_gpd, pprotocal, 0);
					}
#endif
					else{
						//Not defined now
...
{   
    ...
    kal_uint32          nor_Q_hwo_cnt;
	kal_uint32			ack_Q_hwo_cnt;
	...
	nor_Q_hwo_cnt = p_nor_ulq->hwo_rgpd_cnt; //before reload, for debug
	ack_Q_hwo_cnt = p_ack_ulq->hwo_rgpd_cnt; //before reload, for debug
    ...
    ccmni_data_trace(CCMNI_UL_RELOAD_RGPD_IN_HIF, dev->dev_id, nor_Q_hwo_cnt, p_nor_ulq->hwo_rgpd_cnt, p_nor_ulq->max_hwo_rgpd_cnt
															 , ack_Q_hwo_cnt, p_ack_ulq->hwo_rgpd_cnt, p_ack_ulq->max_hwo_rgpd_cnt);
