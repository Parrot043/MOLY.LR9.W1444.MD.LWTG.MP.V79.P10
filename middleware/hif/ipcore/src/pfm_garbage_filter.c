    ...
    kal_uint32              length)
{
    ...
    if (IPC_HDR_IS_V4(p_packet)) {
        ...
        hif_trace_error(PFM_TR_GARBAGE_FILTER_NON_TCP_PACKET, head_gpd);
        goto free_gpd;
    }

    /* Allocate a UL GPD */
    ul_gpd = QBM_ALLOC_ONE(QBM_TYPE_HIF_UL_TYPE_S);
    if (!ul_gpd) {
...
void pfm_garbage_filter_register_parser(void *buf, kal_uint32 filter_cnt, kal_bool uplink)
{
    ...
    for (idx = 0; idx < filter_cnt; idx++) {
        ...
        if ( (IPC_IP_TYPE_IPV4 != raw_filters[idx].ip_type) ||
             (IPC_HDR_PROT_TCP != raw_filters[idx].protocol) ) {
