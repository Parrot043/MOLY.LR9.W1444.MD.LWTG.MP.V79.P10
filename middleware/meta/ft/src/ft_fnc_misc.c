...
void ft_misc_cal_data_write_to_nvram_cnf_from_el1(ilm_struct *ptrMsg)
{
    ilm_struct ilm_ptr;
    kal_uint8 i, uRet = 0;
    kal_uint16 pdu_length = 0, u2_checksum = 0;
    bool bValid = false;
	...
	sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1]");
	tst_sys_trace(g_FT_debug_buf);
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)


    if(i4_ft_cur_misc_op == FT_MISC_OP_CALDATA_INTEGRITY_START_REC)
    {
        req = (ft_cal_data_add_one_req_struct*)ptrMsg->local_para_ptr;
        if(req != NULL)
        {
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
			sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1] req->lid_count = %d", req->lid_count);
			tst_sys_trace(g_FT_debug_buf);
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)
            if(ptrMsg->peer_buff_ptr != NULL)
            {
                pReqData = (FT_CAL_DATA_CHECK_REQ_T*)get_peer_buff_pdu( ptrMsg->peer_buff_ptr, &pdu_length );

#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
			    sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1] pdu_length = %d", pdu_length);
			    tst_sys_trace(g_FT_debug_buf);
				sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1] sizeof(FT_CAL_DATA_CHECK_REQ_T) = %d", sizeof(FT_CAL_DATA_CHECK_REQ_T));
				tst_sys_trace(g_FT_debug_buf);						
				for(i = 0; i < pdu_length; i++ )
				{
				    sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1] pReqData[%d] = %02X", i, pReqData[i]);
					tst_sys_trace(g_FT_debug_buf);	  
     		    }
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)        
                ...
                {
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
					          sprintf((kal_char *)g_FT_debug_buf, "[Update NVRAM cnf from EL1] pReqData->lid = %d, pReqData->rid = %d",  pReqData->lid, pReqData->rid);
					          tst_sys_trace(g_FT_debug_buf);
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)

                    // get the nvram checksum from nvram
                    bValid = ft_misc_cal_data_get_checksum((kal_uint16)pReqData->lid,
                                 pReqData->rid, &u2_checksum);
                    if(bValid)
                    {
                        // update the misc_buffer
                        ft_misc_cal_data_update_local_buf((kal_uint16)pReqData->lid,
                                 pReqData->rid, &u2_checksum);
                    }
                    else
                    {
...
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
	sprintf((kal_char *)g_FT_debug_buf, "[Prepare send cnf to EL1]");
	tst_sys_trace(g_FT_debug_buf);
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)
    ...
    cnf->status = uRet; 
    FT_SEND_MSG(MOD_FT, MOD_ETSTM, DHL_ETSTM_SAP, MSG_ID_FT_CAL_DATA_ADD_ONE_CNF, &ilm_ptr);
/*
    FT_ALLOC_MSG(&ilm_ptr, sizeof(ft_cal_data_add_one_cnf_struct));
    cnf = (ft_cal_data_add_one_cnf_struct*)ilm_ptr.local_para_ptr;
    cnf->status = uRet; 
    FT_SEND_MSG(MOD_FT, MOD_ETSTM, ETSTM_FT_SAP, MSG_ID_FT_CAL_DATA_ADD_ONE_CNF, &ilm_ptr);
 */
#if defined(__MTK_INTERNAL__) && defined(_DEBUG)
	sprintf((kal_char *)g_FT_debug_buf, "[Send cnf to EL1] ilm_ptr.status = %d", cnf->status);
	tst_sys_trace(g_FT_debug_buf);
#endif // #if defined(__MTK_INTERNAL__) && defined(_DEBUG)
